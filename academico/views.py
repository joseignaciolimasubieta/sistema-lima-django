...
def detalle_curso(request, curso_id):
    ...
    if curso.fecha_inicio:
        ...
        if ' A ' in dias_texto:
            ...
            if len(partes) == 2:
                idx_ini = dias_map.get(partes[0].strip(), -1)
                idx_fin = dias_map.get(partes[1].strip(), -1)
                if idx_ini != -1 and idx_fin != -1:
                    if idx_ini <= idx_fin:
                        dias_validos.update(range(idx_ini, idx_fin + 1))
    ...
    if request.method == 'POST':
        ...
        if monto_pago > 0:
            ...
            if nuevo_saldo == 0:
                ...
                # 2. GUARDAMOS (El Cerebro Autocurable registrará automáticamente el total en los Ingresos de hoy)
                inscripcion.save()
...
def asistencia_empleados(request):
    ...
    # --- 2. CÁLCULO DE PORCENTAJES PARA LA BARRA DEL DASHBOARD ---
    total_registros = sum(totales_mes.values())
    porcentajes = {}
    for key, val in totales_mes.items():
        porcentajes[key] = (val / total_registros * 100) if total_registros > 0 else 0
    ...
    return render(request, 'asistencia_empleados.html', {
        ...
        'totales_mes': totales_mes,        # <--- ENVIAMOS TOTALES AL HTML
        'porcentajes': porcentajes,        # <--- ENVIAMOS PORCENTAJES AL HTML
        'total_registros': total_registros # <--- ENVIAMOS REGISTROS AL HTML
    })
@csrf_exempt
@login_required
...
# ==============================================================
# --- MÓDULO DE TAREAS (PÁGINA INDEPENDIENTE) ---
# ==============================================================
@login_required
@user_passes_test(es_administrador)
